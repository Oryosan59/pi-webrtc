...
void on_ice_candidate(GstElement *webrtc_elem, guint mline_idx,
...
void on_ws_message(SoupWebsocketConnection *conn, SoupWebsocketDataType type,
  ...
  if (g_strcmp0(msg_type, "answer") == 0) {
    ...
    JsonObject *ice_obj =
        json_node_get_object(json_parser_get_root(ice_parser));

    const gchar *candidate =
        json_object_get_string_member(ice_obj, "candidate");
    gint sdp_mline_index = json_object_get_int_member(ice_obj, "sdpMLineIndex");

    g_signal_emit_by_name(webrtc, "add-ice-candidate", sdp_mline_index,
                          candidate);
...
gboolean setup_pipeline() {
  ...
  pipeline = gst_parse_launch(
      ...
      "framerate=30/1,width=1920,height=1080 ! "
      "h264parse ! "
...
int main(int argc, char *argv[]) {
  ...
  soup_session_websocket_connect_async(session, msg, nullptr, nullptr, nullptr,
